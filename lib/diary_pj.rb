...
class Diary
  ...
  def find_best_entry_for_reading_time(wpm, minutes)
    @entries.select { |entry| entry.reading_time(wpm) <= wpm * minutes }.max_by { |entry| entry.count_words }
    # == @entries.select { |entry| entry.reading_time(wpm) <= wpm * minutes }.max_by(&:count_words)
